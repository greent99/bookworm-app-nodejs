...
module.exports = {
    ...
    async getAll(req, res) {
        ...
        if (rating != null) {
            ...
            const tempSql = `select books.id from books join reviews on books.id = reviews.book_id group by books.id having avg(cast(rating_start as int)) >= ${rating}`
            rating_condition[sequelize.Op.in] = sequelize.literal('(' + tempSql + ')')
            conditions.id = rating_condition
        }
        const sortType = req.query.sortType || 'onSale'
